...
The `_brand.yml` file includes imports for the [Noto Sans](https://fonts.google.com/noto/specimen/Noto+Sans) family of fonts from Google Fonts. This enables automatically downloading any missing fonts for proper display on the website. 

## Testing Light and Dark Mode

The light and dark mode colors are defined in CSS rules in `theme.scss` and `theme-dark.scss`. The OEDA theme currently uses `flatly` and `darkly` as the base theme, and applies ODHS branding on top of the theme.
...
The toggle for dark content and light content with images in Quarto uses the `{.light-content}` and `{.dark-content}` divs, along with `scss rules` set in the `theme.scss` and `theme-dark.scss` files. 

::: {.light-content}

![OEDA ICON](/assets/oeda-icon-fixed.png){width=20% fig-align="left"}

![OEDA WIDE LOGO](/assets/oeda-logo-wide-light-fixed.svg){width=40% fig-align="left"}

![OEDA VERTICAL LOGO](/assets/oeda-logo-primary-fixed.svg){width=40% fig-align="left"}

:::

::: {.dark-content}

![OEDA ICON](/assets/oeda-icon-dark-fixed.png){width=20% fig-align="left"}

![OEDA WIDE LOGO](/assets/oeda-logo-wide-dark-fixed.svg){width=40% fig-align="left"}

![OEDA DARK LOGO](/assets/oeda-logo-primary-dark-fixed.svg){width=40% fig-align="left"}
