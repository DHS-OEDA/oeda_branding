...
::: {.light-content}
This text is semi-dynamic, and changes color based on the mode. It is currently in **light** mode.

![OEDA icon in PNG format](/assets/oeda-icon-fixed.png)

![OEDA logo](/assets/oeda-logo-wide-light-fixed.svg)

![OEDA Vertical Logo](/assets/oeda-logo-light.svg)

:::

::: {.dark-content}
This text is semi-dynamic, and changes color based on the mode. It is currently in **dark** mode.

![OEDA icon in PNG format, dark version](/assets/oeda-icon-dark-fixed.png)

![OEDA logo, dark version](/assets/oeda-logo-wide-dark-fixed.svg)

![OEDA Vertical Logo, dark version](/assets/oeda-logo-dark.svg)
...
### Custom CSS and Quarto 1.7+

When using Quarto version 1.7 or higher, the `renderings` option replaces all need for custom css when it comes to dark and light mode content. Using custom CSS causes `renderings` to break most of the time. You must choose one approach or the other.
...
scatter_example <- ggplot(mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  ...
  scale_color_oeda_discrete(palette = "warm")
```
...
scatter_example + theme_oeda_light() # for light theme
scatter_example + theme_oeda_dark() # dark theme
...
bar_example + theme_oeda_light() # for light theme
bar_example + theme_oeda_dark() # for dark theme
